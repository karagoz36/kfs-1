...
# macOS has no native toolchain, so the build runs inside a container
...
$(ISO): $(ISO_DIR)/boot/$(NAME) grub/grub.cfg
	@if [ -z "$(GRUB_MKRESCUE)" ]; then \
		echo "ERROR: grub-mkrescue not found. On macOS use 'make docker'."; \
		exit 1; \
	fi
...
# --- For macOS: build inside a Linux container ---
docker-image:
